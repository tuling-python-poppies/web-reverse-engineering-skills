...
description: >-
  先排除非协议任务：Camoufox/浏览器截图、点击、QA、回归测试、页面自动化，普通 REST/GraphQL API client，skill/opencode/MCP 配置，前端/CSS/格式化/重命名/安全头/市场介绍，都不触发；即使出现 Camoufox、GraphQL、WebSocket、protobuf、Imperva、AST 或浏览器字样，只要没有协议恢复目标，也不要触发。仅在用户明确要恢复、定位、验证、交付、逆向、还原、抓入口、实现或协议复现 Web/小程序协议行为时触发；协议信号包括 sign/token/header/cookie/challenge/JSVMP/WASM/验证码/Akamai Bot Manager/River Security/瑞数/Reese84/响应解码/字体映射/会话协议，或 browser-free Python collector。PZDS goodsPublic/page + Aliyun V2 的 InitCaptchaV2/UploadLog/Log2/Log3/VerifyCaptchaV2/T001/F001 + 业务重放任务直接按 collector 处理。明确要求实现时按 collector 交付处理。统一授权、分类、侦察路由后，再按需读取内部 Chromium+CloakBrowser、Camoufox、WeChat、hook、AST、verifier、akamai、river-security、reese84、iv8、python-node、edge-sandbox、pure-python 或 Python delivery Provider。单点 hook/入口定位/已知 AST、Node/jsdom 补环境、EdgeSandbox 传感器工件、iv8 工件、纯 Python signer、完整验证码协议复现、River Security 412/$_ts/S-T Cookie、Akamai sensor/cookie 状态机、Reese84 challenge/cookie/x-d-token 状态机或已有抖音 BDMS 纯 Python 维护也从本入口走快速路径，不升全链路 collector。
...
| Known implementation boundary or explicit implementation Provider request | Use the named Provider route only when the boundary/artifact is named, such as `route: browser-hooks`, `route: ast`, `route: python-node` with `strategy: env-patch`, `route: edge-sandbox`, `route: iv8`, or `route: pure-python`; otherwise stay evidence first. |
...
| Full browser-compatible local runtime for Akamai/Kasada sensors | `edge-sandbox` | implementation mode | `references/providers/implementation/edge-sandbox/PROVIDER.md`; project-local npm package `edge-sandbox`, final live egress still belongs to Python |
