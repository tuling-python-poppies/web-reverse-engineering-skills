...
Chromium ladder: Chrome DevTools visible clean baseline (ordinary Web default first step, after recon gates) -> Chrome parked -> js-reverse normal Chrome for mutation/source/breakpoints (explicit `launch_browser`; headless preferred for that pass unless the user asks for a visible ordinary browser) -> visible CloakBrowser only after explicit selection or fingerprint/observer evidence -> close js-reverse before Camoufox. Skip the visible DevTools baseline only when the user explicitly requires no ordinary window / background-only recon (then document that skip and start the js-reverse pass). For the js-reverse normal-Chrome pass: do not rely on auto-launch/CLI default; residual headful after `close_browser` requires another explicit headless launch before browser actions; brief relaunch flash is allowed, persistent headful after acceptance is a tooling blocker.
