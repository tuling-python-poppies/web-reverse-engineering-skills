...
Chromium recon (mandatory paired pass + optional Cloak): after recon gates, every fresh Chromium target must complete a **mandatory lightweight paired pass** before writing the final collector or claiming full protocol proof — (1) `chrome-devtools` visible clean baseline (page state, redirects, first network view), (2) park Chrome, (3) `js-reverse` mutation/source/initiator pass (explicit `launch_browser`; headless preferred for that pass unless the user asks for a visible ordinary browser). Do not skip either tool unless a real external blocker is reported, or a documented exception applies (evidence-reuse / pure offline local-proof / non-Chromium route). User “no ordinary window / background-only” may skip the **visible DevTools window** only; the js-reverse half remains required and must be documented. After the paired pass, escalate to visible CloakBrowser only on explicit 指纹/Cloak/stealth wording or recorded fingerprint/observer evidence; close js-reverse before Camoufox. For the js-reverse normal-Chrome pass: no auto-launch/CLI default; residual headful after `close_browser` needs another explicit headless launch; brief relaunch flash is allowed, persistent headful after acceptance is a tooling blocker.
