...
- Four migrated python-node historical cases still record `sourceCommit` as
  `95be929~1`; that revision does not resolve in the current Git repository, so a
  full immutable commit cannot be reconstructed locally.
