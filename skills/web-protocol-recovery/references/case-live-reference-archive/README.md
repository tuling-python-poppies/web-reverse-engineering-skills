...
`MANIFEST.json` is the machine-readable integrity boundary. It records every archive path, SHA-256, byte size, exact source commit, and reviewed sensitive literal. `scripts/verify_case_hashes.py` requires disk/manifest parity, byte equality with the declared Git source blob, and explicit review of detected non-empty sensitive literals.
