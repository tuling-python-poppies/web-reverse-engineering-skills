...
All cases use `web-protocol-recovery-case/v1`. Historical restorations use `verificationClass=historical-user-attested`, `selectableAs=template`, and always require fresh target verification. New writeback uses `verificationClass=freshly-verified`, `selectableAs=proof`, with executable test and JSON evidence artifacts.
