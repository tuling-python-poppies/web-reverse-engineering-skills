...
def build_environment(page_url):
    ...
    return {'location': {'ancestorOrigins': {}, 'href': page_url, 'origin': f'{parsed.scheme}://{parsed.netloc}', 'protocol': f'{parsed.scheme}:', 'host': parsed.netloc, 'hostname': parsed.hostname or '', 'port': str(parsed.port or ''), 'pathname': parsed.path or '/', 'search': f'?{parsed.query}' if parsed.query else '', 'hash': f'#{parsed.fragment}' if parsed.fragment else ''}, 'navigator': {'userAgent': UA, 'platform': 'Win32', 'language': 'zh-CN', 'languages': ['zh-CN', 'zh', 'en'], 'webdriver': False}, 'screen': {'width': 1920, 'height': 1080, 'availWidth': 1920, 'availHeight': 1040}}
...
def extract_rs_script_url(html, base_url):
    patterns = ['<script[^>]+src=[\\"\']([^\\"\']+\\.js[^\\"\']*)[\\"\'][^>]*\\br=[\\"\']m[\\"\']', '<script[^>]+\\br=[\\"\']m[\\"\'][^>]+src=[\\"\']([^\\"\']+\\.js[^\\"\']*)[\\"\']']
    for pattern in patterns:
        match = re.search(pattern, html, re.I)
        if match:
            return urllib.parse.urljoin(base_url, match.group(1))
    srcs = re.findall('<script[^>]+src=[\\"\']([^\\"\']+\\.js[^\\"\']*)[\\"\']', html, re.I)
    if srcs:
...
def base_headers(referer=PAGE_URL):
    return {'User-Agent': UA, 'Accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8', 'Accept-Language': 'zh-CN,zh;q=0.9,en;q=0.8', 'Cache-Control': 'no-cache', 'Pragma': 'no-cache', 'Connection': 'keep-alive', 'Referer': referer, 'Upgrade-Insecure-Requests': '1'}

def api_headers(referer=PAGE_URL):
    return {'User-Agent': UA, 'Accept': 'application/json, text/plain, */*', 'Accept-Language': 'zh-CN,zh;q=0.9,en;q=0.8', 'Cache-Control': 'no-cache', 'Pragma': 'no-cache', 'Connection': 'keep-alive', 'Content-Type': 'application/json', 'Origin': 'https://ec.chng.com.cn', 'Referer': referer, 'Sec-Fetch-Dest': 'empty', 'Sec-Fetch-Mode': 'cors', 'Sec-Fetch-Site': 'same-origin', 'X-Requested-With': 'XMLHttpRequest'}

def load_rs_page(ctx, name, page_url, html, js_url, js_code, response):
    snapshot = {'baseURL': page_url, 'html': html, 'headers': [[k, v] for k, v in response.headers.items()], 'resources': {js_url: js_code}}
...
def fetch_rs_stage(session, page_url, index, cookie_text=''):
    ...
    js_response = (_reject_case_live_egress('session.get'), None)[1]
...
def build_body(start):
    return {'start': start, 'limit': PAGE_SIZE, 'type': ANNOUNCEMENT_TYPE, 'search': KEYWORD, 'ifend': IFEND}
