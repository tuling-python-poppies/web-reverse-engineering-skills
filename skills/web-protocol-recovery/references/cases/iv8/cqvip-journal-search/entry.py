...
def build_environment(page_url):
    ...
    return {'location': {'ancestorOrigins': {}, 'href': page_url, 'origin': f'{parsed.scheme}://{parsed.netloc}', 'protocol': f'{parsed.scheme}:', 'host': parsed.netloc, 'hostname': parsed.hostname or '', 'port': str(parsed.port or ''), 'pathname': parsed.path or '/', 'search': f'?{parsed.query}' if parsed.query else '', 'hash': f'#{parsed.fragment}' if parsed.fragment else ''}, 'navigator': {'userAgent': UA, 'platform': 'Win32', 'language': 'zh-CN', 'languages': ['zh-CN', 'zh', 'en'], 'webdriver': False}}

def base_headers():
    return {'User-Agent': UA, 'Accept-Language': 'zh-CN,zh;q=0.9', 'Cache-Control': 'no-cache', 'Pragma': 'no-cache'}
...
def extract_rs_script_url(page_url, html_text):
    match = re.search('src=[\\"\']([^\\"\']+\\.js)[\\"\'][^>]*r=[\\"\']m[\\"\']', html_text)
    if not match:
        match = re.search('src=[\\"\']([^\\"\']+\\.js)[\\"\']', html_text)
    if not match:
...
def refresh_ruishu_cookie(session, page_tag):
    page_headers = {**base_headers(), 'Accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8', 'Referer': PAGE_URL}
    ...
    js_code = (_reject_case_live_egress('session.get'), None)[1].text
    save_text(f'cqvip_rs_source_code_{page_tag}.js', js_code)
    snapshot = {'baseURL': PAGE_URL, 'html': response.text, 'headers': [[k, v] for k, v in response.headers.items()], 'resources': {js_url: js_code}}
...
def build_search_model(page):
    return {'ObjectType': 1, 'SearchKeyList': [{'FieldIdentifier': 'U', 'SearchKey': KEYWORD, 'PreLogicalOperator': None, 'AfterLogicalOperator': None, 'LeftBracket': None, 'RighgtBracket': None, 'IsExact': None, 'ClusterShowName': None}], 'SearchExpression': None, 'BeginYear': None, 'EndYear': None, 'UpdateTimeType': None, 'JournalRange': None, 'DomainRange': None, 'ClusterFilter': '', 'ClusterLimit': 0, 'ClusterUseType': 'Article', 'UrlParam': '', 'Sort': '0', 'SortField': None, 'UserID': None, 'PageNum': page, 'PageSize': PAGE_SIZE, 'SType': None, 'StrIds': None, 'IsRefOrBy': 0, 'ShowRules': f'  任意字段={KEYWORD}  ', 'IsNoteHistory': 0, 'AdvShowTitle': None, 'ObjectId': None, 'ObjectSearchType': 0, 'ChineseEnglishExtend': 0, 'SynonymExtend': 0, 'ShowTotalCount': 0, 'AdvTabGuid': ''}

def build_payload(page):
    model = build_search_model(page)
    return {'searchParamModel': json.dumps(model, ensure_ascii=False, separators=(',', ':')), 'pageNum': str(page), 'pageSize': str(PAGE_SIZE)}

def post_search_list(session, page):
    headers = {**base_headers(), 'Accept': 'application/json, text/javascript, */*; q=0.01', 'Content-Type': 'application/x-www-form-urlencoded; charset=UTF-8', 'Origin': 'https://qikan.cqvip.com', 'Referer': PAGE_URL, 'X-Requested-With': 'XMLHttpRequest'}
...
def print_brief_result(page, text):
    total_match = re.search('id="hidShowTotalCount" value="(\\d+)"', text)
    titles = re.findall('<a[^>]+href="([^"]*/Qikan/Article/Detail\\?id=[^"]+)"[^>]*>(.*?)</a>', text, flags=re.S)[:5]
