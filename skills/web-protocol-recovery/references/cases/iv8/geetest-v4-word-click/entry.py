...
def get(session, url, **kwargs):
    ...
    REQUESTS_USED += 1
    response = session.get(url, timeout=TIMEOUT, allow_redirects=False, **kwargs)
...
def candidate_labels(crop, beta_ocr, classic_ocr):
    ...
    color_mask = Image.fromarray(
        np.where((rgb.max(axis=2) - rgb.min(axis=2)) > 80, 0, 255).astype(np.uint8)
    )
...
def solve_word(background, prompts):
    ...
    crops = [
        image.crop((max(0, x1 - 4), max(0, y1 - 4), min(image.width, x2 + 4), min(image.height, y2 + 4)))
    ...
    for prompt_index in range(len(prompts)):
        ...
        points.append([
            round((x1 + x2) / 2 / image.width * 10000),
            round((y1 + y2) / 2 / image.height * 10000),
        ])
...
def environment():
    # Adapt these fields from one current, same-session browser baseline.
    return {
        "location": {
...
def main():
    ...
    summary = {
        "provider": CAPTCHA_PROVIDER, "version": CAPTCHA_VERSION,
        "captcha_type": CAPTCHA_TYPE, "protocol_type": CAPTCHA_PROTOCOL_TYPE,
        "prompt_text": prompt_text, "match_methods": methods,
        "verify_has_w": bool(parse_qs(urlparse(verify_url).query).get("w")),
        "status": result.get("status"), "result": response_data.get("result"),
        "fail_count": response_data.get("fail_count"), "score": response_data.get("score"),
