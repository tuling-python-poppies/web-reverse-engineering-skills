...
headers = {'Accept': '*/*', 'Accept-Language': 'zh-CN,zh;q=0.9,en;q=0.8', 'Cache-Control': 'no-cache', 'Connection': 'keep-alive', 'Pragma': 'no-cache', 'Referer': 'https://login-user.kugou.com/', 'Sec-Fetch-Dest': 'script', 'Sec-Fetch-Mode': 'no-cors', 'Sec-Fetch-Site': 'cross-site', 'User-Agent': UA}
environment = {'location': {'href': PAGE_URL, 'origin': 'https://login-user.kugou.com', 'protocol': 'https:', 'host': 'login-user.kugou.com', 'hostname': 'login-user.kugou.com', 'port': '', 'pathname': '/login/', 'search': '?appid=1014&ref=https://www.kugou.com/reg/web/&redirect_uri=https://staticssl.kugou.com/common/html/login/regok.html&callback=UsLoginCallback', 'hash': ''}, 'navigator': {'userAgent': UA, 'platform': 'Win32', 'language': 'zh-CN', 'languages': ['zh-CN', 'zh', 'en'], 'webdriver': False}, 'screen': {'width': 1920, 'height': 1080, 'availWidth': 1920, 'availHeight': 1040, 'colorDepth': 24}}
...
def main():
    ...
    params = {'aid': AID, 'protocol': 'https', 'accver': '1', 'showtype': 'popup', 'ua': 'TW96aWxsYS81LjAgKFdpbmRvd3MgTlQgMTAuMDsgV2luNjQ7IHg2NCkgQXBwbGVXZWJLaXQvNTM3LjM2IChLSFRNTCwgbGlrZSBHZWNrbykgQ2hyb21lLzEwNC4wLjAuMCBTYWZhcmkvNTM3LjM2', 'noheader': '1', 'fb': '1', 'aged': '0', 'enableAged': '0', 'enableDarkMode': '0', 'grayscale': '1', 'clientype': '2', 'cap_cd': '', 'uid': '', 'lang': 'zh-cn', 'entry_url': 'https://login-user.kugou.com/login/', 'elder_captcha': '0', 'js': '/tcaptcha-frame.5bae14dd.js', 'login_appid': '', 'wb': '1', 'subsid': '1', 'callback': '', 'sess': ''}
    ...
    puzzle_cfg = next((fg for fg in dyn_info['fg_elem_list'] if fg.get('move_cfg', {}).get('data_type')))
    ...
    bg_data = (_reject_case_live_egress('requests.get'), None)[1].content
    sprite_data = (_reject_case_live_egress('requests.get'), None)[1].content
    if SAVE_RAW_CACHE:
        ensure_cache_dir()
    ...
    tdc_js = (_reject_case_live_egress('requests.get'), None)[1].text
    if SAVE_RAW_CACHE:
        ensure_cache_dir()
    ...
    ans_data = json.dumps([{'elem_id': 1, 'type': 'DynAnswerType_POS', 'data': f'{gap_x + 10},{init_pos_y}'}])
    verify_data = {'collect': collect, 'tlg': str(len(collect)), 'eks': eks, 'sess': sess, 'ans': ans_data, 'pow_answer': f'{pow_prefix}{pow_ans}', 'pow_calc_time': str(pow_time)}
    verify_resp = (_reject_case_live_egress('requests.post'), None)[1]
