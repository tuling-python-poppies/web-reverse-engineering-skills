...
def build_headers(page_url=PAGE_URL):
    return {'user-agent': UA, 'accept': 'application/json, text/plain, */*', 'accept-language': 'zh-CN,zh;q=0.9,en;q=0.8', 'content-type': 'application/x-www-form-urlencoded', 'origin': 'https://www.zhipin.com', 'referer': page_url, 'x-requested-with': 'XMLHttpRequest'}

def build_data(page):
    return {'scene': '1', 'query': KEYWORD, 'city': CITY_CODE, 'experience': '', 'degree': '', 'industry': '', 'scale': '', 'salary': '', 'jobType': '', 'page': str(page), 'pageSize': str(PAGE_SIZE)}
...
def build_security_environment(security_url):
    ...
    return {'location': {'href': security_url, 'origin': 'https://www.zhipin.com', 'protocol': 'https:', 'host': 'www.zhipin.com', 'hostname': 'www.zhipin.com', 'port': '', 'pathname': parsed.path or '/web/common/security-check.html', 'search': f'?{parsed.query}' if parsed.query else '', 'hash': ''}, 'navigator': {'userAgent': UA, 'platform': 'Win32', 'language': 'zh-CN', 'languages': ['zh-CN', 'zh', 'en'], 'webdriver': False}, 'screen': {'width': 1920, 'height': 1080, 'availWidth': 1920, 'availHeight': 1040, 'colorDepth': 24, 'pixelDepth': 24}, 'window': {'origin': 'https://www.zhipin.com', 'devicePixelRatio': 1}, 'canvas': {'fingerprint': {'toDataURL': {'png': 'data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVR42mP8z8BQDwAFgwJ/lQn8YQAAAABJRU5ErkJggg=='}}}}
...
def compute_stoken(session, headers, challenge_payload):
    ...
    security_url = 'https://www.zhipin.com/web/common/security-check.html?' + urllib.parse.urlencode({'seed': seed, 'name': name, 'ts': str(ts), 'callbackUrl': '', 'srcReferer': PAGE_URL})
    html_page = f'<!DOCTYPE html>\n<html><head><meta charset="utf-8"></head><body>\n<script src="{js_url}"></script>\n</body></html>'
    save_text('zhipin_security_check.html', html_page)
    snapshot = {'baseURL': security_url, 'html': html_page, 'headers': [[key, value] for key, value in headers.items()], 'resources': {js_url: js_code}}
    with _iv8().JSContext(environment=build_security_environment(security_url), config={'timezone': 'Asia/Shanghai'}) as ctx:
        ...
        token = ctx.eval(f'encodeURIComponent((new window.ABC).z({json.dumps(seed, ensure_ascii=False)}, {ts}));')
