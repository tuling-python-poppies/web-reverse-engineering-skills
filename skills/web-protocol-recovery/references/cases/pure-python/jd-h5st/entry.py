...
def build_body(*, page: int=1, page_count: int=20, start_pos: int | None=None) -> str:
    ...
    inner = {'pagenum': page, 'pagecount': page_count, 'startpos': start_pos, 'ptag': '', 'sku': '', 'cid1': '', 'cid2': '', 'cid3': ''}
    outer = {'func': 'item_rec', 'recpos': 6163, 'param': compact_json(inner), 'clientPageId': '', 'clientVersion': '2.0'}
...
def jd_se_data(text: str) -> str:
    ...
    for index in range(6):
        if index < 5:
            ...
            chunk = text[offset:]
        suffix.append(JD_SE_ALPHABET[sum((ord(char) for char in chunk)) & 15])
...
def request_algo(*, session: Any | None=None) -> dict[str, Any]:
    ...
    payload = {'version': H5ST_VERSION, 'fp': fp, 'appId': APP_ID, 'timestamp': now_ms(), 'platform': 'web', 'expandParams': '', 'fv': FILE_VERSION, 'localTk': ''}
    headers = {'User-Agent': UA, 'Accept': 'application/json, text/plain, */*', 'Accept-Language': 'zh-CN,zh;q=0.9', 'Origin': 'https://m.jd.com', 'Referer': PAGE_URL}
    response = (_reject_case_live_egress('session.post'), None)[1]
    ...
    match = re.search('var\\s+rd=[\'\\"]([^\'\\"]+)[\'\\"]', algo)
    if not result.get('tk') or not match:
        raise RuntimeError(f'unexpected request_algo response: {data!r}')
    return {'source': 'live', 'tk': str(result['tk']), 'fp': str(result.get('fp') or fp), 'algoTs': str((data.get('data') or {}).get('ts') or payload['timestamp']), 'rd': match.group(1)}

def build_env_segment(fp: str, timestamp_ms: int, *, random_values: tuple[str, str, str] | None=None) -> str:
    ...
    env = {'sua': 'Windows NT 10.0; Win64; x64', 'pp': {}, 'extend': {'wd': 0, 'l': 0, 'ls': 5, 'wk': 0, 'bu1': '0.1.9', 'bu3': 27, 'bu4': 0, 'bu5': 0, 'bu6': 3, 'bu7': 0, 'bu8': 0, 'random': extend_random, 'bu12': -8, 'bu10': 14, 'bu11': 4}, 'pf': 'Win32', 'random': random_value, 'v': FILE_VERSION, 'bu4': '0', 'canvas': jd_md5(''), 'webglFp': '', 'ccn': 8, 'bu14': bu14, 't': timestamp_ms, 'fp': fp}
    return jd_base64_encode(compact_json(env))

def sign_h5st(body: str, material: dict[str, Any], *, timestamp_ms: int | None=None, env_timestamp_ms: int | None=None, env_random_values: tuple[str, str, str] | None=None) -> tuple[str, dict[str, str]]:
    ...
    h5st = ';'.join([sign_time, fp, APP_ID, tk, signature, H5ST_VERSION, str(timestamp_ms), env, env_digest, ENV_TAIL])
...
def fetch_products(body: str, h5st: str, *, session: Any | None=None) -> tuple[dict[str, Any], bytes]:
    ...
    if session is None:
        session = _reject_case_live_egress('requests.Session')
    headers = {'User-Agent': UA, 'Accept': 'application/json, text/plain, */*', 'Accept-Language': 'zh-CN,zh;q=0.9', 'Origin': 'https://m.jd.com', 'Referer': PAGE_URL, 'x-referer-page': PAGE_URL, 'x-rp-client': 'h5_1.0.0'}
...
def parse_products(payload: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    for item in content:
        ...
        products.append({'sku': str(sku) if sku is not None else '', 'name': str(name or ''), 'price': str(item.get('price') or item.get('jdPrice') or item.get('oriprice') or ''), 'jdPrice': str(item.get('jdPrice') or ''), 'oriprice': str(item.get('oriprice') or ''), 'link': str(item.get('link') or ''), 'img': f"{item.get('imgprefix') or ''}{item.get('imgbase') or ''}", 'cate1': ext.get('cate1') if ext else None, 'cate2': ext.get('cate2') if ext else None, 'cate3': ext.get('cate3') if ext else None, 'shopId': ext.get('shopId') if ext else None, 'brandId': ext.get('brandId') if ext else None})
    return products

def write_products_csv(products: list[dict[str, Any]], path: str | Path) -> None:
    with Path(path).open('w', newline='', encoding='utf-8-sig') as handle:
        fieldnames = ['sku', 'name', 'price', 'jdPrice', 'oriprice', 'link', 'img', 'cate1', 'cate2', 'cate3', 'shopId', 'brandId']
...
def run(*, live: bool=False, page: int=1, page_count: int=20) -> dict[str, Any]:
    ...
    if not live:
        return {'status': 'offline-proof', 'bodySha256': body_sha256(body), 'jdSha256Abc': jd_sha256('abc'), 'jdBase64Abc': jd_base64_encode('abc')}
    pass
    session = _reject_case_live_egress('requests.Session')
    ...
    return {'status': 'success', 'runtime': 'pure-python', 'httpStatus': 200, 'responseBytes': len(raw), 'productCount': len(products), 'bodySha256': sign_meta['bodySha256'], 'h5stLength': len(h5st), 'h5stPrefix': h5st[:48], 'products': products}
