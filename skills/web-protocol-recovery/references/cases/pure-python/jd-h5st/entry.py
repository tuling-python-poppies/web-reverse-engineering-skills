...
# Historical full implementation:
#   references/case-live-reference-archive/pure-python/jd-h5st/entry.py
...
def _reject_case_live_egress(action: str = "live HTTP") -> None:
    raise RuntimeError(
        ...
        "Historical code: references/case-live-reference-archive/pure-python/jd-h5st/entry.py. "
...
def request_algo(*, session: Any | None = None) -> dict[str, Any]:
    _reject_case_live_egress("request_algo live HTTP")
...
def fetch_products(body: str, h5st: str, *, session: Any | None = None) -> tuple[dict[str, Any], bytes]:
    _reject_case_live_egress("fetch_products live HTTP")
    raise AssertionError("unreachable")
...
def run(*, live: bool = False, page: int = 1, page_count: int = 20) -> dict[str, Any]:
    ...
    return {
        ...
        "note": "offline-only; historical live entry in case-live-reference-archive",
    }
