...
Scope schemes may be `http`, `https`, `ws`, or `wss`; WebSocket scheme support authorizes the handshake URL only. Sent frames remain separate `websocketFrame` budget units and require the same session, channel, and frame acceptance boundary. A discovery-only scope may observe bounded browser traffic and newly revealed hosts, but it cannot authorize Python replay, sent frames, mutation, or persistence; the hub must mint a new exact replay scope from the observed route.
