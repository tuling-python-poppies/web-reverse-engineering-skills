...
```json
{
  ...
  "authorization": {
    ...
    "requestBudget": {"total": 100, "remaining": 100, "minDelayMs": 0, "concurrency": 1, "automaticObservationStopThreshold": 200, "observedAutomatic": {"total": 0, "byKind": {"redirect": 0, "subresource": 0, "xhrFetch": 0, "beaconPing": 0, "eventSource": 0, "websocket": 0}, "destinations": []}},
    "artifactPolicy": {
...
Standing approval (SKILL Non-Negotiables item 7) applies only inside the selected shape. For routine work, record `actionClass=read-only`, or `verifier-submit` only when a protocol-needed verifier round is the selected action; auto-record browser flags, `liveReplayAllowed=true`, `requestBudget.total=100` and `requestBudget.remaining=100` when the user supplied no budget, one positive immutable request budget, redacted/nonsecret cache artifact fields, supplied-session use, and the resolved absolute cwd when unset. This default does not authorize a larger shape, mutation, dependency installation, target-code execution, raw-secret handling, or case writeback.
...
Scope schemes may be `http`, `https`, `ws`, or `wss`; WebSocket scheme support authorizes the handshake URL only. Sent frames remain separate `websocketFrame` budget units and require the same session, channel, and frame acceptance boundary. Optional `authorization.discoveryScopes` list read-only observation scopes for browser recon: they may observe bounded browser traffic and newly revealed hosts, but they authorize no Provider-initiated replay, sent frames, mutation, or persistence. After observation, the hub must mint a new exact scope in `allowedHostsAndRoutes` before any live request. External runtimes may be attached read-only with `runtimeCustody.leaseMode=observe-external`: attach, target/network/source reads, and MCP-owned debug-state cleanup are allowed; starting, stopping, reconfiguring, or taking over an externally owned process is not. External operational state stays outside project `allowedPaths`.
...
All budget and limit fields are integers: `total >= 0`, `0 <= remaining <= total`, `minDelayMs >= 0`, `concurrency >= 1`; threshold `0` denies Chrome baseline. If the user gives no budget, the hub chooses `requestBudget.total=100` and `remaining=100` once before first egress. The hub chooses any other `total` once before first egress. `total`, `remaining`, and `observedAutomatic` are shared task state: route handoffs cannot reset them, and neither hub nor Provider may replenish/increase `total` without `scope-expansion` confirmation. Providers enforce delay/concurrency before egress.
...
Before executing target-supplied code, hash the exact reviewed bytes and require `targetCodeExecution=local-only` or `approved-reviewed-hash` after the user's executionPolicy confirmation, a matching `approvedCodeSha256`, and for reviewed-hash mode unexpired approval evidence and deadline. Execution must go through the Provider-owned bounded process launcher: fixed argv (`node` plus one hash-bound asset), a project `js_reverse_cache` cwd, bounded stdin/stdout/stderr, and a hard timeout. `node:vm` and a work-order supplied argv are not security boundaries. A template that has no bundled launcher must fail closed rather than invoke a user-declared command. Optional `executionPolicy.sandbox` may record external capability denial metadata but is not a required gate. Dependency installation requires `dependencyInstall=approved` and an exact command present in `approvedCommands`; package names alone are not approval.
...
```json
{
  ...
  "requestBudget": {"total": 100, "priorRemaining": 100, "consumed": 0, "remaining": 100, "byKind": {"navigation": 0, "request": 0, "retry": 0, "websocketHandshake": 0, "websocketFrame": 0}, "observedAutomatic": {"total": 0, "byKind": {"redirect": 0, "subresource": 0, "xhrFetch": 0, "beaconPing": 0, "eventSource": 0, "websocket": 0}, "destinations": []}, "minDelayMsApplied": 0, "maxConcurrencyObserved": 1},
