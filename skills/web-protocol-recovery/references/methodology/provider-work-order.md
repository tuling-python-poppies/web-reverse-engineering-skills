...
WPR owns the reverse task from intake through final acceptance. Internal providers never become a second task owner. They receive one bounded work order, produce evidence or code, clean up runtime resources they created, and return control to WPR.
...
6. WPR accepts or rejects the result against `acceptanceTest`; a provider's `complete` label is not sufficient.
