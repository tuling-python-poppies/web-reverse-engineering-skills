...
`scripts/gates/validate_architecture.py` keeps this contract executable by simulating the official routing chains as explicit path lists, including evidence reuse, Chromium/hooks, Chromium→AST→env-patch→collector, AST/env-patch, verifier implementation variants, Akamai, Akamai→edge-sandbox, River Security→iv8, River Security→env-patch, `Reese84 -> iv8/python-node -> python-collector`, selected cases, and iv8 API inventory gates. Each simulated chain must reference existing files, keep every window within its cap, and stay within the 24-path whole-task cap after de-duplication.
