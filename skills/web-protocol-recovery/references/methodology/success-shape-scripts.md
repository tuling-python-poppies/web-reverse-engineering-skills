...
3. Read one implementation provider only when needed: `browser-hooks`, `ast`, `env-patch`, `iv8`, `verifier`, `akamai`, `river-security`, `douyin-abogus-native`, or `python-collector`. For captcha, read `verifier/PROVIDER.md` first, then at most one selected family reference. For Akamai, read `akamai/PROVIDER.md` first, then at most one selected Akamai reference. For River Security, read `river-security/PROVIDER.md` first and select at most one registry case. A self-contained deterministic decode may remain inline.
