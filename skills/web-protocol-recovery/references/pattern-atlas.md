...
| Kasada `KP_UIDz` / `x-kpsdk-*` / `window.KPSDK` / double-UUID `ips.js` / `/tl` gate business replay | separate `p.js` wrapper from `ips.js` collector, reproduce the runtime bytecode, run browser-free filling the read environment surfaces, mint `x-kpsdk-ct` on one coherent IP/UA/TLS egress; treat `x-kpsdk-cd` as an open PoW blocker | `references/providers/protocol-recovery/kasada/PROVIDER.md` |
