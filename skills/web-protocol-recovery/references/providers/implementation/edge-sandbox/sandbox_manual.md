...
**说明：** 本文档中的 `<edge-sandbox-root>` 指 EdgeSandbox 的安装目录（本文档所在目录）。在实际使用时，请将 `<edge-sandbox-root>` 替换为您的实际安装路径，例如 `D:\develop_software\edge_node_sandbox`。

当前验证基线：

- Node.js `24.11.0`
- Node V8 `13.6.233.10-node.28`
...
### 1.1 必须使用 Node 24.11.0

项目要求精确的 Node 版本。不要使用 PATH 中的 Node 22、Node 20 或其他版本代替。

确认 Node 24.11.0 已安装并可用：
...
如果系统 PATH 中的 node 不是 24.x，可通过以下方式切换：

- **NVM**: `nvm use 24`
- **FNM**: `fnm use 24`
- **手动**: 将 Node 24.11.0 的安装目录加入 PATH 首位

Python 宿主脚本可通过 `NVM_HOME` 环境变量自动定位 Node 24，无需手动切换。

Node 版本错误时，可能出现 `module.hasTopLevelAwait is not a function` 等启动错误。这类错误不能作为项目实现失败的证据，应先修正 Node 版本。
...
import {
  EdgeSandbox,
  edge150Fingerprint,
} from "@edge-node-sandbox/runtime";
```
...
| 导出 | 作用 |
| --- | --- |
| `EdgeSandbox` | 创建和控制沙箱实例 |
| `edge150Fingerprint` | 冻结的 Edge 150 默认 profile |

### 2.3 第一次验证

```powershell
node --test tests/behavior/fingerprint-timing-profile.test.js
```
...
用户代码超过 `timeoutMs` 时，控制器会终止整个 child process，并为后续请求启动干净的执行单元。不要依赖超时后的全局变量、DOM 对象、Promise 或 Worker 继续存在。
...
### 17.3 child-process 超时和隔离测试

```powershell
node --test tests/isolation/timeout.test.js
node --test tests/isolation/node-leak.test.js
```
...
- Window：`1232` 名称，缺失/多余/顺序/descriptor 差异为 0；
- function rows：`11449`；
- function UTF-16 长度：`7005820`；
- function FNV-1a64：`c6cd7456e4838d96`；
- DOM factories：`232/232`；
- Worker observations：`718/718`。

### 17.5 Worker Thread benchmark
...
### 18.1 `module.hasTopLevelAwait is not a function`
...
必须是 `v24.11.0`。如果不是，请切换到 Node 24：

- **NVM**: `nvm use 24`
- **FNM**: `fnm use 24`
- **手动**: 确保 Node 24.11.0 在 PATH 中
...
4. `npm run audit:functions` 的行数和 digest 是否与基线一致。

默认 Edge 150 的 function baseline 必须保持不变。
