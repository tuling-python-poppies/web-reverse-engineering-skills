/**
  * @env-module performance-timing
 * @description Performance API模拟
 * @compatibility Chrome 80+, Firefox 75+, Edge 79+
 */

(function() {
    const profile = window.__ProfileManager__ ? window.__ProfileManager__.getSection('performance') : {};
    const runtimeStart = Date.now();
    const timeOrigin = profile.timeOrigin || runtimeStart;
    const startTime = timeOrigin;
    const nowOffset = profile.nowOffset || 0;
    const entries = [];
    const marks = {};
    const measures = {};
    ...
    const performance = {
        ...
        // 高精度时间
        now: function() {
            return Date.now() - runtimeStart + nowOffset;
        },
