/**
  * @env-module element-mock-monitor
 * @description 增强版环境监控系统 - 支持灵活mock、方法调用链监控、返回值控制
 * @status 高级可选模块。日常优先使用 core/proxy-access-monitor.js。
 * @use-when 需要以下能力之一：
 ...
 * @not-for 只需要基础 watch/safefunction 的常规补环境流程 → 用 proxy-access-monitor.js
 * @conflict 与 core/classified-env-monitor.js 互斥（二者均注册 window.__EnvMonitor__），不要同时加载。与 core/proxy-access-monitor.js 不建议混用（功能重叠）。
 * @alternatives core/proxy-access-monitor.js（首选，所有模块的基础依赖）、core/classified-env-monitor.js（侧重日志系统 + 查询 API + 分类 mock 存储）
