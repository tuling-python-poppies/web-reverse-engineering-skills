...
(() => {
    ...
    // 删除并重写 toString
    Object.defineProperty(myToString, 'name', { value: 'toString', configurable: true });
    nativeSources.set(myToString, 'function toString() { [native code] }');
    delete Function.prototype['toString'];
    set_native(Function.prototype, "toString", myToString);
    
    // 导出安全函数设置
    globalThis.safefunction = (func) => {
        if (typeof func !== 'function') throw new TypeError('safefunction expects a function');
        nativeSources.set(func, `function ${func.name || ''}() { [native code] }`);
        return func;
    };
    ...
    const watchedObjects = new WeakMap();
