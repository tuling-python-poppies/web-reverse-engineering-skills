...
1. 诊断报告没有相关 `moduleLoadErrors` 或 `undefinedPaths`。
2. 外部 hook 看不到可疑访问。
...
5. `vm-browser-gap-diagnose.js` 会等待目标脚本直接返回的 Promise，并确定性执行 timeout 回调及每个 interval 回调一次；它仍不会自动触发 DOM 事件、XHR hook、Worker 消息或被目标内部 try/catch 吞掉的异步分支。依赖真实时间间隔、重复 interval 或外部事件的功能仍需专用 harness 验证。
