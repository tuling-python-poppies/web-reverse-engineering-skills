...
顶部放可编辑常量和缓存目录。写入前置条件（工作单给出绝对 `projectRoot`、`layout=web-protocol-recovery-simple`、非 `no-write` 的 `writeMode` 和精确 `allowedPaths`）见 `references/methodology/provider-work-order.md`。`offline-skeleton` 是验证模式，不是写入模式；`no-write` 时只在回复中给代码，不运行文件模板。
...
项目根、统一项目形态与禁止目录以 `references/methodology/project-layout.md` 和当前工作单为准；用户提供的旧项目/旧目录只当输入证据读取。iv8 只能写这些路径：
