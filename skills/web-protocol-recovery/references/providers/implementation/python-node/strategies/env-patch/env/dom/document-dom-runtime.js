...
(function() {
    'use strict';
    let DOMException = typeof window.DOMException === 'function' ? window.DOMException : null;
    ...
    if (!DOMException) {
        ...
        const requireState = value => {
            const metadata = state.get(value);
            if (!metadata) throw new TypeError('Value of this must be of type DOMException');
            return metadata;
        };
        DOMException = function DOMException(message = '', name = 'Error') {
            ...
            state.set(instance, { message: String(message), name: String(name) });
        ...
        Object.defineProperties(DOMException.prototype, {
            ...
            name: { get() { return requireState(this).name; }, enumerable: true, configurable: true },
            message: { get() { return requireState(this).message; }, enumerable: true, configurable: true },
            code: { get() { return nameToCode[requireState(this).name] || 0; }, enumerable: true, configurable: true },
            [Symbol.toStringTag]: { value: 'DOMException', configurable: true }
        });
        for (const [constant, code] of Object.entries(constants)) {
        ...
        if (typeof window.safefunction === 'function') {
            window.safefunction(DOMException);
            for (const key of ['name', 'message', 'code']) {
                window.safefunction(Object.getOwnPropertyDescriptor(DOMException.prototype, key).get);
            }
        }
