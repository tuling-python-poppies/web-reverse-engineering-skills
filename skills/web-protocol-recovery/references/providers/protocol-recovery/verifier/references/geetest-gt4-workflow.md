...
2. work-order 必须绑定当前 bundle SHA-256、ISO-8601 未过期 approval deadline 和受控执行边界；`task` 等无期限哨兵不能替代 deadline。
3. iv8 只读取同轮 bundle，生成 `w` 或其他明确窄工件。
4. 执行边界必须同时确认 webpack registry 存在且目标导出为 callable；模块编号（例如 `32`）仍只在当前 bundle hash 范围内有效。
...
如果没有可验证的受控执行边界，停止在静态证据或纯 Python 路径；不要用 `node:vm`、自报 sandbox 标签或任意命令替代执行边界证明。
...
| iv8 加载异常 | API 成员、bundle hash、DOM/环境最小适配和受控执行边界；不把页面 UI 自动化当最终交付 |
