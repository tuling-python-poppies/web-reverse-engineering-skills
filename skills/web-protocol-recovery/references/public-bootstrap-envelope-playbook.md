...
Use this playbook when a page is publicly visible but successful replay depends on passive public bootstrap data, entry-route cookies, or an encrypted outer envelope.
...
If JavaScript/WASM/font execution derives client state, a challenge response must run, or a runtime getter/egress already emits the artifact, route to `challenge-state-envelope-playbook.md` instead.
...
2. Freeze the bootstrap artifact.
   Save the exact public key, config blob, nonce seed, side asset, or wrapper metadata returned by the bootstrap endpoint. Record source and hash; do not execute it here.
...
- Verify that the bootstrap remains passive. Any executable state transition belongs to the challenge-state playbook.
