...
- `references/providers/protocol-recovery/kasada/PROVIDER.md` when Kasada-native markers such as `KP_UIDz` cookies, `x-kpsdk-ct`/`x-kpsdk-cr`/`x-kpsdk-st` headers, a `window.KPSDK` bootstrap, a double-UUID `p.js`/`ips.js` path, a `/tl` sensor POST, or a `reporting.cdndex.io` beacon have independent corroboration. The Provider owns `p.js`/`ips.js` role separation, the `/fp` context and `/tl` submission, browser-free VM sandbox execution, `x-kpsdk-ct` session binding, and the `x-kpsdk-cd` proof-of-work boundary. A `429` alone is not enough.
