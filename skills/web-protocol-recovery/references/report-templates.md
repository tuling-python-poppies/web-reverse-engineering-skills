...
|---|---|---|
| Canonical contracts and machine-facing guidance in `SKILL.md`, `PROVIDER.md`, playbooks, and `references/**` prose | English | Stable fields, route decisions, and Provider handoffs are asserted across docs and evals. |
| Machine keys and enum values: `route`, `shape`, `gateFamily`, `family`, `selectableAs`, `verificationClass`, JSON field names | English, lowercase, stable | Asserted verbatim by `evals/route-regression.json` and `scripts/gates/validate_architecture.py`. Never translate or alias these. |
